...
public final class BrowserScreen extends Screen {
    ...
    public BrowserScreen(Screen parent) {
        this(parent, HOME_URL);
    }

    /** 指定首页的构造：斗地主这类 App 直接给服务端的内嵌页面地址 */
    public BrowserScreen(Screen parent, String homeUrl) {
        super(Component.translatable("mcphone.app.browser"));
        this.parent = parent;
        this.homeUrl = homeUrl == null ? HOME_URL : homeUrl;
    }
    ...
    protected void init() {
        ...
        if (browser == null && failure == null) {
            ...
            if (!backend.isAvailable()) {
                ...
                browser = backend.create(homeUrl, toPx(viewW), toPx(viewH));
