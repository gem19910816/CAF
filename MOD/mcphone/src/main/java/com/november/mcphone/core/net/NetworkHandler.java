...
public final class NetworkHandler {
    ...
    public static void register() {
        ...
        // C2S: 玩家在手机里点了「生存档案」。照旧只追加在末尾，不动 PROTOCOL_VERSION
        MCphoneNetwork.registerToServer(
                com.november.mcphone.feature.caf.net.OpenCafStatusPacket.class,
                com.november.mcphone.feature.caf.net.OpenCafStatusPacket::encode,
                com.november.mcphone.feature.caf.net.OpenCafStatusPacket::decode,
                NetworkHandler::handleOpenCafStatus
        );

        // C2S: 玩家在手机里点了「商店」。照旧只追加在末尾，不动 PROTOCOL_VERSION
        MCphoneNetwork.registerToServer(
                com.november.mcphone.feature.shop.net.OpenShopPacket.class,
                com.november.mcphone.feature.shop.net.OpenShopPacket::encode,
                com.november.mcphone.feature.shop.net.OpenShopPacket::decode,
    ...
    /**
     * 服务端收到：给玩家打开 caf_stamina_core 的状态面板，就是 /caf 那一屏。
     *
     * 校验玩家身上确实带着手机，与 handleOpenEnderChest 同一道闸。装没装
     * caf_stamina_core 由 CafStatusPanel.isLoaded() 判；没装（玩家手机是旧档
     * 带过来的、或伪造包）就静默忽略——那一格在没装的客户端根本不出现。
     */
    /**
     * 服务端收到：给玩家打开 SDM 商店，等价于 /sdmshop open_shop。
     * 手机携带校验与 handleOpenEnderChest 同一道闸。
     */
    private static void handleOpenShop(
            com.november.mcphone.feature.shop.net.OpenShopPacket packet, ServerPlayer player) {
        if (!PhoneItem.isCarriedBy(player)) {
            MCphone.LOGGER.debug("玩家 {} 请求开商店但身上没有手机，已忽略",
                    player.getName().getString());
            return;
        }
        if (com.november.mcphone.feature.shop.client.ShopOpener.isLoaded()) {
            com.november.mcphone.feature.shop.client.ShopOpener.open(player);
        }
    }

    private static void handleOpenCafStatus(
            com.november.mcphone.feature.caf.net.OpenCafStatusPacket packet, ServerPlayer player) {
        if (!PhoneItem.isCarriedBy(player)) {
            MCphone.LOGGER.debug("玩家 {} 请求开生存档案但身上没有手机，已忽略",
                    player.getName().getString());
            return;
        }
        if (com.november.mcphone.feature.caf.client.CafStatusPanel.isLoaded()) {
            com.november.mcphone.feature.caf.client.CafStatusPanel.open(player);
        }
    }
}
